...
mtext("(a)",cex=1.5, side = 3, adj = 0.075, line = -2,font=2)
# Add a legend
legend("topright",pch=c(1,15,16,NA,NA,NA), lty=c(1,1,2,1,1,1),
...
# Add the points for extinction probability within 100, 20, 10 years
...
# Add the lines showing the thresholds for Critically Endnagered, Endangered, and Vulnerable
...
# First, create a blank plot of population size over time.
plot(0,0, type="n", xlim=c(1970,2020), ylim=c(0,max(comm)),
 xlab="Year",las=1, ylab="Population size",
	cex.axis=1.0, cex.lab= 1.3, mgp=c(2.4,0.6,0))
	
# Add background polygons to show the thresholds between extinction risk cateories
...
# Create a matrix that assigns dummy codes to each threat category
rli.mat <- ifelse(comm>VU.t,1,ifelse(comm>EN.t,0.6,ifelse(comm>CR.t,0.4,ifelse(comm>0,0.2,1e-99))))

# Create a matrix for the percentage of species in each threat category
prop.mat <- matrix(0,ncol=length(years), nrow=5)
colnames(prop.mat) <- years; rownames(prop.mat) <- c("LC", "VU","EN", "CR", "EX")

# Run a loop to calcualte the percentage of each species per threat category
...
# Assign colours for each threat category
iucn.col <- c("green", "yellow","orange", "red", "black")
# Cumulate the percentages for the stacked bar chart
cum.mat <- apply(prop.mat,2,cumsum)

# Create a function that plots the changing percentages as steps, rather than straight lines
stepX <- function(x){
...
# First, create a blank plot of percentage threat categories over time.
plot(years,rli, type="n", xlim=c(1970,2020), ylim=c(0,110),
 xlab="Year",las=1, ylab="Pecentage of species (%)",
	cex.axis=1.0, cex.lab= 1.3, mgp=c(2.4,0.6,0)) 
# Add the cumulative stepped polygons
...
# Add gret lines to give the illusion of seperate bars
for(i in 1:length(years)){
	lines(c(years[i],years[i]),c(0,100), col="grey")
}

# Add legend and label plot
...
# Calculate the mean and standard error for the dummy variable of threat categories: Red List Index
rli <- apply(rli.mat,2,mean)
rli.conf <- apply(rli.mat,2,function(x){sd(x)/sqrt(length(x))})

# First, create a blank plot of Red List Index over time.
plot(years,rli, type="n", xlim=c(1970,2020), ylim=c(0,1.1),
 xlab="Year",las=1, ylab="Red List Index",
	cex.axis=1.0, cex.lab= 1.3, mgp=c(2.4,0.6,0))
# Add a polygon for confidence intervals
...
# Close and save the plot device
dev.off()
